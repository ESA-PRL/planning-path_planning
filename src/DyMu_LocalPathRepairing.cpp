    ...
    for (uint j = 0; j < res_ratio; j++)
    {
        for (uint i = 0; i < res_ratio; i++)
        {
            ...
            nodeRow.back()->global_pose.position[0] =
                nodeRow.back()->parent_pose.position[0] - 0.5 + (0.5 / (double)res_ratio)
                + nodeRow.back()->pose.position[0] * (1 / (double)res_ratio);
            nodeRow.back()->global_pose.position[1] =
                nodeRow.back()->parent_pose.position[1] - 0.5 + (0.5 / (double)res_ratio)
...
void DyMuPathPlanner::subdivideGlobalNode(globalNode* gNode)
{
    ...
    for (int i = 0; i < 8; i++)
        if (!(gNode->nb8List[i] == NULL))
            if (!(gNode->nb8List[i]->hasLocalMap)) createLocalMap(gNode->nb8List[i]);
}
                                           ...
                                           base::Time& localTime)
{
    ...
    uint b = (uint)(
        fmin(global_layer.size(), ((wPos.position[1] + (double)height / 2 * res) / global_res)));
    uint c = (uint)(fmax(0, ((wPos.position[0] - (double)width / 2 * res) / global_res)));
    uint d = (uint)(
        fmin(global_layer[0].size(), ((wPos.position[0] + (double)width / 2 * res) / global_res)));
    for (uint j = a; j < b; j++)
        for (uint i = c; i < d; i++) subdivideGlobalNode(global_layer[j][i]);
    ...
    for (uint j = 0; j < height; j++)
        for (uint i = 0; i < width; i++)
        {
            ...
            if ((pos.position[0] > -0.5) && (pos.position[0] < globalSizeX)
                && (pos.position[1] > -0.5)
                && (pos.position[1] < globalSizeY))
            {
                uint8_t value = traversabilityMap.image[j * traversabilityMap.getRowSize()
                ...
                if ((!lNode->isObstacle) && ((value != 0) || (gNode->isObstacle)))
                {
                    ...
                    gNode->hazard_density =
                        std::min(1.0, gNode->hazard_density + 1.0 / (res_ratio * res_ratio));
                    for (uint k = 0; k < 8; k++)
                    {
                        if (gNode->nb8List[k] != NULL)
                        {
                            gNode->nb8List[k]->hazard_density = std::min(
                                1.0,
                                gNode->nb8List[k]->hazard_density + 0.1 / (res_ratio * res_ratio));
                        }
                    }
                }
            }
...
//    is located
int DyMuPathPlanner::repairPath(base::Waypoint wayp_start, uint safe_index, uint index)
{
    ...
    while (
        (index < current_path.size())
        && (sqrt(pow(current_path[index].position[0] - current_path[overtake_index].position[0], 2)
                 + pow(current_path[index].position[1] - current_path[overtake_index].position[1],
                       2))
            < reconnect_distance))
        index++;
    ...
    else
    {
        ...
        else
        {
            ...
            proximity = sqrt(pow(current_path[0].position[0] - wayp_start.position[0], 2)
                             + pow(current_path[0].position[1] - wayp_start.position[1], 2));
            for (uint k = 1; k < index; k++)
            {
                candidate_proximity =
                    sqrt(pow(current_path[k].position[0] - wayp_start.position[0], 2)
                         + pow(current_path[k].position[1] - wayp_start.position[1], 2));
                if (candidate_proximity < proximity) closest_index = k;
            }
            for (uint k = closest_index; k < index; k++)
                original_distance +=
                    sqrt(pow(current_path[k + 1].position[0] - current_path[k].position[0], 2)
                         + pow(current_path[k + 1].position[1] - current_path[k].position[1], 2));
            ...
            if (localPath.size() > 1)
            {
                ...
                {
                    new_distance +=
                        sqrt(pow(localPath[k + 1].position[0] - localPath[k].position[0], 2)
                             + pow(localPath[k + 1].position[1] - localPath[k].position[1], 2));
                }
                globalNode* gNode;
                for (uint k = closest_index; k < index; k++)
                {
                    ...
                    gNode->trafficability =
                        std::min(original_distance / new_distance, gNode->trafficability);
                }
                if (repairing_approach == CONSERVATIVE)
                {
                                         ...
                                         uint& safe_index)
{
    ...
    {
        if (sqrt(pow(obNode->world_pose.position[0] - current_path[i].position[0], 2)
    ...
    if (isBlocked) maxIndex = current_path.size();
...
void DyMuPathPlanner::expandRisk()
{
    ...
    while (!local_expandable_obstacles.empty())
    {
        ...
        /*if (tCurrent.toSeconds() > 1.0)
        {
            ...
            std::cout << "Parent is " << gNode->pose.position[0] << "," << gNode->pose.position[1]
        << std::endl;
            std::cout << "Neighbor parent is " <<nodeTarget->nb4List[3]->parent_pose.position[0] <<
        "," << nodeTarget->nb4List[3]->parent_pose.position[1] << std::endl;
            break;
        }*/
    }
}
...
{
    if (local_expandable_obstacles.empty()) return NULL;
...
localNode* DyMuPathPlanner::computeLocalPropagation(base::Waypoint wayp_start,
                                                    base::Waypoint wOvertake)
{
    // wayp_start is the waypoint from which the path is repaired

    double Tovertake = getTotalCost(wOvertake);
    std::cout << "Overtake is " << wOvertake.position[0] << "," << wOvertake.position[1]
              << " and T associated is " << Tovertake << std::endl;
    double distReference = sqrt(pow(wayp_start.position[0] - wOvertake.position[0], 2)
    ...
    std::cout << "Init Waypoint is " << wayp_start.position[0] << "," << wayp_start.position[1]
              << std::endl;

    local_agent = getLocalNode(wayp_start);

    std::cout << "Local Agent is " << local_agent->pose.position[0] << ","
              << local_agent->pose.position[1] << std::endl;
    ...
    while (true)  // TODO: Control this
    {
        ...
        for (uint i = 0; i < 4; i++)
        {
            ...
            if ((nodeTarget->nb4List[i] != NULL) && (nodeTarget->nb4List[i]->state == OPEN)
                && (!nodeTarget->nb4List[i]->isObstacle))
            {
                propagateLocalNode(nodeTarget->nb4List[i]);
                if (nodeEnd == NULL)
                    if ((nodeTarget->nb4List[i]->total_cost < Tovertake)
                        && (nodeTarget->nb4List[i]->risk == 0))
                        nodeEnd = nodeTarget->nb4List[i];
            }
        }
        if ((nodeEnd != NULL) && (nodeEnd->state == CLOSED)
            && (nodeEnd->nb4List[0]->state == CLOSED)
            && (nodeEnd->nb4List[1]->state == CLOSED)
            && (nodeEnd->nb4List[2]->state == CLOSED)
            && (nodeEnd->nb4List[3]->state == CLOSED))
        {
            std::cout << "Node End is " << nodeEnd->pose.position[0] << ","
                      << nodeEnd->pose.position[1] << "; Risk associated is " << nodeEnd->risk
                      << "; T associated is " << nodeEnd->total_cost << std::endl;
            return nodeEnd;
        }
        tCurrent = base::Time::now() - tInit;
        if (tCurrent.toSeconds() > 5.0)
        {
            ...
            // std::cout << "Node Target is " << nodeEnd->pose.position[0] << "," <<
            // nodeEnd->pose.position[1] << "; Risk associated is " << nodeEnd->risk << "; T
            // associated is " << nodeEnd->total_cost << std::endl;
            std::cout << "The local narrowband size is " << local_narrowband.size() << std::endl;
            std::cout << "The local propagated nodes size is " << local_propagated_nodes.size()
                      << std::endl;
            return NULL;
        }
    }
...
void DyMuPathPlanner::propagateLocalNode(localNode* nodeTarget)
{
    ...
    {
        if (nodeTarget->deviation
            == std::numeric_limits<double>::infinity())  // It is not in narrowband
        {
...
localNode* DyMuPathPlanner::minCostLocalNode(double Tovertake, double minC)
{
    ...
    for (i = 0; i < local_narrowband.size(); i++)
    {
        currentH = local_narrowband[i]
                       ->deviation;  // + fmax(0, local_narrowband[i]->total_cost - Tovertake)/minC;
        if (currentH < minH)
        {
            minH = currentH;
...
localNode* DyMuPathPlanner::minCostLocalNode(localNode* reachNode)
{
    ...
    double distance = sqrt(
        ...
        + pow(local_narrowband.front()->world_pose.position[1] - reachNode->world_pose.position[1],
              2));
    ...
    for (i = 0; i < local_narrowband.size(); i++)
    {
        distance = sqrt(
            ...
            + pow(local_narrowband[i]->world_pose.position[1] - reachNode->world_pose.position[1],
                  2));
                                                          ...
                                                          double tau)
{
           ...
           > 1.5 * local_res)
    {
        ...
        if (sqrt(pow((wPos.position[0] - trajectory[0].position[0]), 2)
                 + pow((wPos.position[1] - trajectory[1].position[1]), 2))
            < 0.01 * tau * local_res)
...
base::Waypoint DyMuPathPlanner::computeLocalWaypointDijkstra(localNode* lNode)
{
    ...
    wPos.heading =
        atan2(newY - lNode->world_pose.position[1], newX - lNode->world_pose.position[0]);
    return wPos;
}
...
bool DyMuPathPlanner::computeLocalWaypointGDM(base::Waypoint& wPos, double tau)
{
    ...
    wPos.position[2] = interpolate(globalDistX,
                                   globalDistY,
                                   gNode00->elevation,
                                   gNode10->elevation,
                                   gNode01->elevation,
    ...
    {
        std::cout << "local waypoint (" << wPos.position[0] << "," << wPos.position[1]
                  << ") is degenerate (nan gradient)" << std::endl;
        return false;
    }
    ...
    {
        std::cout << "local waypoint (" << wPos.position[0] << "," << wPos.position[1]
                  << ") is degenerate (near 0 gradient)" << std::endl;
        return false;
    }
...
void DyMuPathPlanner::gradientNode(localNode* nodeTarget, double& dnx, double& dny)
{
    ...
    if (((nodeTarget->nb4List[1] == NULL) && (nodeTarget->nb4List[2] == NULL))
        || ((nodeTarget->nb4List[1] != NULL) && (nodeTarget->nb4List[2] != NULL)
            && (nodeTarget->nb4List[1]->deviation == std::numeric_limits<double>::infinity())
            && (nodeTarget->nb4List[2]->deviation == std::numeric_limits<double>::infinity())))
        dx = 0;
    else
    {
        ...
        else
        {
            if ((nodeTarget->nb4List[2] == NULL)
                ...
                dx = (nodeTarget->nb4List[2]->deviation - nodeTarget->nb4List[1]->deviation) * 0.5;
        }
    }
    if (((nodeTarget->nb4List[0] == NULL) && (nodeTarget->nb4List[3] == NULL))
        || ((nodeTarget->nb4List[0] != NULL) && (nodeTarget->nb4List[3] != NULL)
            && (nodeTarget->nb4List[0]->deviation == std::numeric_limits<double>::infinity())
            && (nodeTarget->nb4List[3]->deviation == std::numeric_limits<double>::infinity())))
        dy = 0;
    else
    {
        ...
        else
        {
            if ((nodeTarget->nb4List[3] == NULL)
                ...
                dy = (nodeTarget->nb4List[3]->deviation - nodeTarget->nb4List[0]->deviation) * 0.5;
        }
    }
            ...
            else if (isBlocked)  // We enter a safe area after having been blocked
            {
                ...
                while (rectifiedIndex > 0)
                {
                    if (sqrt(pow(current_path[minIndex].position[0]
                                     - current_path[rectifiedIndex].position[0],
                                 2)
                             + pow(current_path[minIndex].position[1]
                                       - current_path[rectifiedIndex].position[1],
                                   2))
                        > 2.0)
                        break;
                    rectifiedIndex--;
...
std::vector<std::vector<double>> DyMuPathPlanner::getRiskMatrix(base::Waypoint rover_pos)
{
    ...
    for (uint i = 0; i < local_side_num; i++) risk_matrix[i].resize(local_side_num);

    /*std::cout << "LocalSideNum = " << local_side_num << std::endl;
    std::cout << "Resolution Ratio = " << res_ratio << std::endl;
    std::cout << "Global Node = " << gNode->pose.position[0] << "," << gNode->pose.position[1] <<
    std::endl;*/
    ...
    for (uint j = 0; j < global_side_num; j++)
        for (uint i = 0; i < global_side_num; i++)
        {
            ...
            // std::cout << "Global Node Position = " << node_target->pose.position[0] << "," <<
            // node_target->pose.position[1] << std::endl;
            if ((node_target != NULL) && (node_target->hasLocalMap))
            {
                ...
                for (uint l = 0; l < res_ratio; l++)
                    for (uint k = 0; k < res_ratio; k++)
                    {
                        ...
                        // std::cout << "Risk Matrix coordinates " << (k+i*res_ratio) << "," <<
                        // (l+j*res_ratio) << std::endl;
                    }
            }
        }
...
std::vector<std::vector<double>> DyMuPathPlanner::getDeviationMatrix(base::Waypoint rover_pos)
{
    ...
    for (uint i = 0; i < local_side_num; i++) dev_matrix[i].resize(local_side_num);
        ...
        for (uint i = 0; i < global_side_num; i++)
        {
            ...
            if ((node_target != NULL) && (node_target->hasLocalMap))
            {
                ...
                for (uint l = 0; l < res_ratio; l++)
                    for (uint k = 0; k < res_ratio; k++)
                    {
                        if (node_target->localMap[l][k]->deviation
